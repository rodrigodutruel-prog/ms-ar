MS AR y 3DDUT AR 4.36.0

- La vista ya no queda borrosa para siempre después de salir y volver a la app (una llamada, WhatsApp, el permiso del micrófono): el tiempo afuera se contaba como si el teléfono no diera abasto y la resolución bajaba un escalón cada vez. Ahora no cuentan las pausas, el arranque al volver ni los trabajos de un momento (apoyar, volcar, tocar, foto, video). Si el teléfono realmente no llega, igual baja, y ahora también cuando está muy cargado (antes, justo ahí, no bajaba nunca).
- Volcar, «Tocar la base» y «Deshacer volcado» ya no traban la vista: el modelo se recorría tres veces en ese cuadro (80-160 ms con 250.000 triángulos) y ahora una sola, sobre una copia rápida de las posiciones. El toque sobre el modelo (ficha, medir, Tocar la base) es unas cinco veces más rápido. Mismos resultados, número por número.
- El ingeniero sigue desde donde estaba cuando se vuelca la pieza (antes aparecía de golpe al principio de su recorrido); si estaba parado mirando la pieza, primero gira en el lugar y después camina.
- La sombra vuelve enseguida después de varios toques seguidos de Volcar (antes se calculaban todas, una detrás de otra), y la cámara abre antes: la sombra inicial se calcula aparte, mientras la cámara arranca. Las fotos ya no esperan detrás de las sombras.
- Enfoque fijo con modelos grandes a escala real: Google recomienda el enfoque fijo para seguir el lugar; con el automático el lente buscaba foco al caminar y la imagen se borroneaba en ese momento. Sobre la hoja, en maqueta, con una pieza chica a escala real (menos de 1 m, se mira de cerca) y mientras se graba video sigue el automático, y cambia solo si se cambia la escala.
- Menos calor en sesiones largas sobre superficie: ARCore entregaba toda la sesión una segunda imagen de 1280x720 que nadie usa (solo la necesita el QR de la hoja); ahora pide la de 640x480, siempre con la misma imagen de fondo y los mismos cuadros por segundo.
- Menos trabajo de la placa gráfica con «Paredes» y con Rayos X o Aire (el pre-paso de profundidad ya no calcula la luz ni la oclusión de cada capa), y las aristas negras no calculan una luz que después no se usa. De paso, donde un equipo real tapa entera la parte de adelante del modelo, ya no asoma medio transparente la de atrás (ni el ingeniero) a más de 4 m.
- Abrir la vista AR es más rápido: el modelo pasa a la vista nativa en la mitad de tandas y con la conversión del navegador, y el QR de la hoja se lee una sola vez por archivo (antes en cada toque de AR, unos 200 ms).
- El Diagnóstico anota qué cámara y qué enfoque se usan y, una vez por minuto cuando pasa algo, los cuadros por segundo reales, cuántas imágenes de cámara se perdieron, los tirones de más de 100 ms y cuánto estuvo enfocando el lente.
- Incluye todo lo de la 4.34.0 y la 4.35.0, que no se publicaron: un equipo real que pasa delante tapa el modelo aunque lo tape entero, la app decide punto por punto si lo que tapa es real y solo afloja lo dudoso; una máquina cerca tapa a un modelo lejos; se ve solo la cara de adelante donde algo tapa a medias; la sombra del modelo y la del ingeniero se tapan con lo real; sin fantasma (cuando la profundidad es dudosa el modelo se muestra entero en vez de quedar medio transparente, sin parpadear); la maqueta sigue la altura de la mesa que ARCore va corrigiendo; y el fijado ya no salta de una solución a otra.

Verificación: auditoría de rendimiento (cinco análisis y una crítica: CPU, placa gráfica, ARCore, arranque y web) y revisión adversarial de la 4.36 (cinco revisores y dos verificadores por hallazgo: 9 confirmados y corregidos —la pausa que se escapaba por el cuadro en curso, la hoja que nunca bajaba la resolución, el ingeniero que arrancaba de costado, el enfoque fijo con piezas chicas y al grabar, y las cifras de la bitácora— y 2 refutados). Pruebas nuevas del ritmo (36: pausas de 0,3 a 30 s, sobrecarga sostenida, diez volcados seguidos, cámara perdida, notas), del volcado (17: igual bit a bit en 1000 giros al azar, toques idénticos, una sola sombra de cinco pedidos, el ingeniero que retoma y gira) y de la cámara y el enfoque (15); 24 errores simulados y cada uno lo detecta su prueba; las 27 baterías nativas, los shaders, las pruebas web y el emulador (las dos marcas abren la vista AR con la 4.36.0 y anotan cámara 640x480 y enfoque). El enfoque, la resolución al volver a la app y el volcado sin tirones quedan para probar en el teléfono.

Instalar cada APK 4.36.0 sobre la anterior, sin desinstalar.

MS AR y 3DDUT AR 4.35.0

- Sin fantasma: cuando la profundidad es dudosa, el modelo ya no queda medio transparente (en la maqueta del taller la oclusión quedó a 0,46). O tapa bien, o se muestra entero: se apaga cuando lo dudoso pasa el 65 % y vuelve recién cuando baja del 50 % durante 0,4 s seguidos, así no parpadea.
- Maqueta sobre la mesa: la base sigue la altura de la mesa que ARCore va corrigiendo (en la prueba, la mesa quedó 3 cm más abajo que al apoyar la maqueta y la maqueta flotaba: al moverse parecía que se deslizaba). Solo en vertical, suave, sin temblar con el ruido (4 mm de zona muerta) y hasta 5 cm. Si se sube o baja a mano con los botones, o se marca el piso, manda eso. Cambiar la escala no la hace saltar.
- Corregido: después de perder el seguimiento o de reubicar, la oclusión podía quedar apagada por un valor viejo del historial.
- El Diagnóstico anota cuándo la oclusión se apaga o se enciende y por qué (la mediana de lo dudoso), y en maqueta dice cuánto sigue la mesa en vez de «el modelo no se movió».
- Incluye todo lo de la 4.34.0 (que no se publicó): un equipo real que pasa delante tapa el modelo aunque lo tape entero, la app decide punto por punto si lo que tapa es real (movimiento de costado del teléfono o puntos de ARCore delante) y solo afloja lo dudoso; una máquina cerca tapa a un modelo lejos; se ve solo la cara de adelante donde algo tapa a medias; la sombra del modelo y la del ingeniero se tapan con lo real; un cuadro sin profundidad no apaga la oclusión; y el fijado ya no salta de una solución a otra.

Verificación: los dos videos del sector plegado (el de la 4.33 muestra el estante por encima de la plegadora y del gabinete negro, lo que corrigió la 4.34); revisión adversarial de la 4.35 (7 hallazgos confirmados y corregidos, 4 refutados); pruebas del juez (48, entre ellas la secuencia de la histéresis y el caso del taller: 67 % dudoso se muestra entero), de la mesa (11) y de la oclusión (21); 8 errores simulados y cada uno lo detecta su prueba; las 24 baterías nativas, los shaders y el emulador (abre la vista AR en maqueta con la 4.35.0). La maqueta sobre una mesa real y la oclusión real quedan para probar en el teléfono.

Instalar cada APK 4.35.0 sobre la anterior, sin desinstalar.

MS AR y 3DDUT AR 4.34.0

- Un equipo real que pasa delante del modelo lo tapa, aunque lo tape entero. La regla de la 4.32, que servía para que el modelo no desapareciera al alejarse, miraba cuánto del modelo quedaba tapado; un equipo grande delante daba la misma cuenta que una profundidad mala, y justo al pasar frente a la máquina la oclusión se apagaba y el modelo quedaba dibujado encima, como si se moviera. Ahora la app decide punto por punto si lo que tapa es creíble: si el teléfono se movió de costado respecto de ese punto (así la profundidad por movimiento mide bien) o si ARCore sigue puntos del entorno delante del modelo, lo tapado es real y se tapa siempre. Solo afloja cuando es dudoso: caminando hacia el modelo o alejándose sin nada que lo confirme (el caso de la 4.31), o cuando se sigue viendo la pared de atrás donde la profundidad dice que hay algo delante.
- Una máquina cerca tapa a un modelo lejos: si lo que tapa está a menos de 3 m y claramente delante, la oclusión ya no se apaga por la distancia del modelo (antes, con el modelo a más de 4-6 m, se veía a través de la máquina).
- Donde algo real tapa a medias, se ve solo la cara de adelante del modelo (antes se sumaban las caras de atrás y el "fantasma" quedaba mucho más visible), y las aristas negras se tapan más rápido.
- La sombra del modelo y la del ingeniero se tapan con lo real (antes quedaban pintadas sobre la máquina y se deslizaban con el piso).
- Un cuadro sin profundidad ya no apaga la oclusión de golpe (se usa la anterior hasta 2 cuadros), un cuadro con ruido no la tira, y al volver a mirar el modelo no arrastra lo de antes.
- Fijado: un ancla que ARCore corre justo en el límite ya no hace saltar el modelo de una solución a otra; una vez descartada vuelve recién cuando coincide 1 s seguido.
- El Diagnóstico anota, solo cuando cambia algo (como mucho 8 notas), qué parte tapa lo real, cuánto es dudoso, a qué distancia está lo que tapa, cómo se movía el teléfono, los puntos de ARCore delante y detrás, y cuán nueva es la profundidad.

Verificación: diagnóstico con cuatro análisis y una crítica (simulación de la pasada frente a una máquina: en la 4.33 el modelo quedaba encima 3,5 de los 5,5 s); revisión adversarial del cambio (16 hallazgos confirmados y corregidos, 9 refutados). Pruebas nuevas del juez (29: hacia atrás sin puntos se afloja, el mismo mapa de costado tapa siempre, puntos delante confirman, puntos detrás contradichos vetan, máquina lisa pasando frente a una pared con puntos, lo firme hasta 9 m y con sensor, sus topes de 3 m y 30 %, quieto conserva, lo que entra o vuelve a entrar a la pantalla no hereda, salto de ARCore, hueco de profundidad al caminar, ruido 45 %: fuerza media 0,97 contra 0,73 de la 4.33, imágenes repetidas, vuelta con máquina confirmada, caso mixto y costo 0,12 ms por cuadro); cinco errores simulados y cada uno lo detecta su prueba; fijado con histéresis (la punta de la pieza no salta, antes 1,9 cm por cuadro); las 23 baterías nativas, los shaders y el emulador (abre la vista AR con la 4.34.0). La oclusión real y los puntos de ARCore quedan para probar en el teléfono (el emulador no tiene profundidad).

Instalar cada APK 4.34.0 sobre la anterior, sin desinstalar.

MS AR y 3DDUT AR 4.33.0

- Fijado firme: el modelo ya no cuelga de una sola ancla de ARCore. Queda sostenido por el ancla principal y una en cada esquina de su planta, en el piso, y su posición y su giro salen de las posiciones de todas (ARCore corrige bien las posiciones y mal el giro de un ancla sola). Con el mismo ruido de ARCore, la punta de una pieza de 3 m se movía en promedio 2,15 cm y ahora 0,36 cm. El modelo queda siempre derecho aunque el ancla se incline, y si una sola ancla se corre por su cuenta se descarta. Cada vez que se toca «Fijar» las anclas se ponen en las esquinas de donde quedó la pieza.
- «Marcar piso», en Ajustar: con la pieza parada como va (volcada o con «Tocar la base»), se apunta el ⊕ al piso real junto a la pieza y se toca: la base de la pieza queda ahí y se descarta lo que se había subido o bajado con los botones. Ese piso es el plano 0 para todo: sombra, ingeniero, choques y la altura que dice la ficha. El punto marcado suma un ancla más al fijado.
- La pieza volcada se entiende como quedó parada: la ficha da las medidas de la pieza parada (ancho, alto y largo como está), el corte «alto» es horizontal de verdad y la escala crece desde el centro de la pieza parada.
- El ingeniero inspecciona de verdad. Para frente al centro de cada cara y en las esquinas (antes paraba cada 1,2 m y giraba el cuerpo 90° y de vuelta en cada parada), MIRA PUNTOS REALES de la pieza con la cabeza, de uno en uno, y analiza: se acerca a ver un detalle inclinándose, se agacha si lo que mira está abajo, mira hacia arriba si está alto, señala apuntando al punto con el brazo, anota en la planilla levantando la vista, piensa mirando el conjunto o retrocede para ver la pieza entera. Gira como una persona (antes se daba vuelta en 0,6 s), no se desliza parado, caminando mira adelante y echa miradas a la pieza, y ya no se abre en diagonal hacia un paso sin salida. Con la pared y la plegadora al lado recorre toda la parte libre y se da vuelta antes de llegar (antes se quedaba clavado en el único punto libre).

Verificación: pruebas nuevas del fijado (16: lugar exacto, un ancla que cambia de orientación no gira el modelo, un ancla que se corre 10 cm se descarta, anclas juntas, ninguna viva, ruido de ARCore en 2.000 cuadros), del piso (6), del corte con la pieza volcada (4) y del ingeniero (la mirada apunta al punto a menos de 6°, giro máximo 170°/s, sin deslizarse, caminando no mira para atrás, y todas las acciones de análisis); las 22 baterías nativas, el shader, el web (45) y el puente web → vista nativa (94). Las poses se revisaron dibujándolas (señalar a la altura del punto, agacharse, mirar arriba, anotar, pensar, acercarse, retroceder) y el recorrido simulado con la pared y la plegadora. En el emulador la vista AR abre con «Marcar piso» en Ajustar. El fijado con anclas y marcar el piso sobre el piso real quedan para probar en el teléfono (el emulador no detecta el piso).

Instalar cada APK 4.33.0 sobre la anterior, sin desinstalar.

MS AR y 3DDUT AR 4.32.0

- El modelo ya no desaparece al alejarse: «Paredes» puede tapar una parte del modelo (una columna, una máquina, la esquina de una pared), pero si la profundidad dice que lo tapa casi entero, se muestra igual. En teléfonos sin sensor de profundidad (la calculan por movimiento), caminando hacia atrás mientras se mira el modelo esa medida falla justo en el centro de la imagen, donde está el modelo, y lo borraba entero. Además, en esos teléfonos «Paredes» se va apagando entre 4 y 6,5 m, donde la medida ya no sirve (con sensor, entre 5 y 8 m como antes).
- El ingeniero vuelve a verse con «Alinear a pared» y con el replanteo sin hoja: la pared marcada con los dos puntos es un límite firme y él camina solo del lado libre. Antes la espalda del modelo quedaba contra la pared y el recorrido de atrás, a 60 cm, caía ADENTRO de la pared real: el ingeniero arrancaba ahí, la pared lo tapaba y no se lo veía nunca. Ahora aparece del lado libre, camina por delante y los costados, y se da vuelta antes de llegar a la pared. Si con Ajustar el modelo se separa de la pared y queda lugar atrás, vuelve a rodearlo entero. Funciona también en teléfonos sin profundidad (la pared se conoce por los dos puntos).
- El Diagnóstico dice ahora si el teléfono mide la profundidad con sensor o por movimiento, cuándo la oclusión dejó ver el modelo porque la profundidad parecía equivocada («profundidad dudosa: tapaba el N % del modelo a X m») y cuándo el ingeniero pasó al lado libre de la pared.

Verificación: pruebas nuevas de la oclusión (23: pared detrás, algo delante de todo el modelo, una columna delante de un tercio, sin datos, de lejos, fuera de pantalla, bajada en el acto y vuelta en ~0,8 s, puntos del modelo repartidos por área), de la pared marcada (organizador de 1,2 x 0,5 m contra la pared: arranca del lado libre, 5 minutos sin pasar detrás de la pared ni por la plegadora de al lado, sigue caminando e inspeccionando; con lugar atrás lo rodea entero), las 21 baterías nativas, el shader, el web (45) y el puente web → vista nativa (94). En el emulador la 4.32.0 abre la vista AR con un modelo y pide el punto 1 de la pared. La oclusión con profundidad real y marcar los dos puntos quedan para probar en el teléfono (el emulador no tiene profundidad ni detecta el piso).

Instalar cada APK 4.32.0 sobre la anterior, sin desinstalar.
